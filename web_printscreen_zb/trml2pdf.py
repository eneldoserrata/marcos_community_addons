    ...
    def __init__(self, nodes, localcontext):
        ...
        for node in nodes:
            ...
            for style in node.findall('paraStyle'):
                ...
                self.styles_obj[sname] = reportlab.lib.styles.ParagraphStyle(sname, self.default_style["Normal"],
                                                                             **self.styles[sname])
...
class _rml_doc(object):
    ...
    def render(self, out):
        ...
        if len(el):
            pt_obj = _rml_template(self.localcontext, out, el[0], self, images=self.images, path=self.path,
                                   title=self.title)
            ...
            pd_obj = _rml_canvas(self.canvas, self.localcontext, None, self, self.images, path=self.path,
                                 title=self.title)
            pd_obj.render(pd)
...
class _rml_canvas(object):
    ...
    def _circle(self, node):
        self.canvas.circle(x_cen=utils.unit_get(node.get('x')), y_cen=utils.unit_get(node.get('y')),
                           r=utils.unit_get(node.get('radius')),
                           **utils.attr_get(node, [], {'fill': 'bool', 'stroke': 'bool'}))

    def _place(self, node):
        flows = _rml_flowable(self.doc, self.localcontext, images=self.images, path=self.path, title=self.title).render(
            node)
...
class _rml_draw(object):
    ...
    def render(self, canvas, doc):
        canvas.saveState()
        cnv = _rml_canvas(canvas, self.localcontext, doc, self.styles, images=self.images, path=self.path,
...
class _rml_Illustration(platypus.flowables.Flowable):
    ...
    def draw(self):
        drw = _rml_draw(self.localcontext, self.node, self.styles, images=self.self2.images, path=self.self2.path,
...
class _rml_flowable(object):
    ...
    def _table(self, node):
        ...
        table = platypus.LongTable(data=data, colWidths=colwidths, rowHeights=rowheights,
    ...
    def _flowable(self, node, extra_style=None):
        ...
        if node.tag == 'para':
            ...
            try:
                ...
                return None
            args = utils.attr_get(node, [], {'ratio': 'float', 'xdim': 'unit', 'height': 'unit', 'checksum': 'int',
                                             'quiet': 'int', 'width': 'unit', 'stop': 'bool', 'bearers': 'int',
            ...
            return platypus.Image(image, mask=(250, 255, 250, 255, 250, 255),
            ...
            return platypus.flowables.HRFlowable(width=width_hr, color=color.get(color_hr),
                                                 thickness=float(thickness_hr), lineCap=str(lineCap_hr))
        else:
...
class _rml_template(object):
    def __init__(self, localcontext, out, node, doc, images=None, path='.', title=None):
        ...
        self.doc_tmpl = TinyDocTemplate(out, pagesize=pageSize, **utils.attr_get(node, ['leftMargin', 'rightMargin',
                                                                                        'topMargin', 'bottomMargin'],
                                                                                 {'allowSplitting': 'int',
                                                                                  'showBoundary': 'bool',
                                                                                  'rotation': 'int', 'title': 'str',
                                                                                  'author': 'str'}))
        self.page_templates = []
        self.styles = doc.styles
        self.doc = doc
        ...
        for pt in pts:
            ...
            for frame_el in pt.findall('frame'):
                frame = platypus.Frame(**(utils.attr_get(frame_el,
                                                         ['x1', 'y1', 'width', 'height', 'leftPadding', 'rightPadding',
                                                          'bottomPadding', 'topPadding'],
                                                         {'id': 'str', 'showBoundary': 'bool'})))
